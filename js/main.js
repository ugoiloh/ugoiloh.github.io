// Loading the idb promised library
...
// idb promised library code ends
...
// creating the Idb to cache the list of currencies
const loadCurrency = (() => {

    let curr = document.getElementsByClassName("postfix");



    let dbPromise = idb.open('curr-db1', 1, function(upgradeDb) {
        switch (upgradeDb.oldVersion){
            case 0:
       
        let store1 = upgradeDb.createObjectStore('Mycurr', {
          keyPath: "id"
      });
    ...
    // loading the listing of currencies from the idb database
              ...
              dbPromise.then((db)=>{
                if (!db) return
                    ...
                    let store1 = db.transaction('Mycurr','readonly').objectStore('Mycurr');
                ...
                for (let i = 0;i < message.length - 1;i++) {
                  ...
                  let key = (message[i]).id;
                  options = options + '<option>' + key + '</option>'
                 

                  curr[0].innerHTML = options;
